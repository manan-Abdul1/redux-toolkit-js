...
export const editTask = (editTaskObj) => async (dispatch) => {
    try {
       await axios.put("http://localhost:5500/api/editTask", editTaskObj , {
        headers: {
            Authorization: `Bearer ${getToken()}`
        }
       }).then(response => dispatch(editTodoTask(response.data)))
    } catch (error) {
        console.error(error.response.data.error)
    }
};
export const completeTask = (taskId) => async (dispatch) => {
    try {
        ...
        ).then(response => dispatch(completedTodoTask(response.data.taskId)));
    } catch (error) {
        console.error(error.response.data.error);
    }
};
...
export const deleteTask = (taskId) => async (dispatch) => {
        try {
            await axios.delete(`http://localhost:5500/api/deleteTask?taskId=${taskId}`, {
               ...
               }).then(response => dispatch(deleteTodoTask(response.data.taskId)))
        } catch (error) {
            console.error(error.response.data.error)
        }
    };
